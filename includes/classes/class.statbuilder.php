...
class statbuilder
{
	...
	private function SetNewRanks()
	{
		...
		foreach($this->Unis as $Uni)
		{
			...
			$Ach			= array();
			$Rank           = 1;
			$RankQry        = $GLOBALS['DATABASE']->query("SELECT s.id_owner FROM ".STATPOINTS." as s, ".USERS." as u WHERE s.universe = '".$Uni."' AND s.stat_type = '1' AND s.id_owner = u.id ".((Config::get('stat') == 2)?'AND u.authlevel < '.Config::get('stat_level').' ':'')." ORDER BY ach_points DESC;");
			while ($CurUser = $GLOBALS['DATABASE']->fetch_array($RankQry))
			{
				$Ach[$CurUser['id_owner']]	= $Rank;
				$Rank++;
			}
			...
			$Ach			= array();
			$Rank           = 1;
			$RankQry        = $GLOBALS['DATABASE']->query("SELECT s.id_owner FROM ".STATPOINTS." as s WHERE s.universe = '".$Uni."' AND  s.stat_type = '2' ORDER BY ach_points DESC;");
			while ($CurUser = $GLOBALS['DATABASE']->fetch_array($RankQry))
			{
				$Ach[$CurUser['id_owner']]	= $Rank;
				$Rank++;
	...
	final public function MakeStats()
	{
		...
		while($UserData	= $GLOBALS['DATABASE']->fetch_array($TotalData['Users']))
		{
			...
			(isset($UserData['old_ach_rank']) ? $UserData['old_ach_rank'] : 0).", ".
			(isset($UserPoints[$UserData['id']]['achie']['points']) ? min($UserPoints[$UserData['id']]['achie']['points'], 1E50) : 0).", ".
			(isset($UserPoints[$UserData['id']]['achie']['count']) ? $UserPoints[$UserData['id']]['achie']['count'] : 0)."), ";
		}
		...
		{
			$AllySQL = "INSERT INTO ".STATPOINTS." (id_owner, id_ally, stat_type, universe, tech_old_rank, tech_points, tech_count, build_old_rank, build_points, build_count, defs_old_rank, defs_points, defs_count, fleet_old_rank, fleet_points, fleet_count, total_old_rank, total_points, total_count, ach_old_rank, ach_points, ach_count) VALUES ";
